defmodule Poeticoins.Historical do
  ...
  @spec get_last_trade(pid() | atom(), Product.t()) :: nil | Trade.t()
  def get_last_trade(pid \\__MODULE__, product) do
    GenServer.call(pid, {:get_last_trade, product})
  end

  @spec get_last_trades(pid() | atom(), [Product.t()]) :: nil | [Trade.t()]
  ...
  def handle_call({:get_last_trade, product}, _source, historical) do
    trade = Map.get(historical.trades, product)
    {:reply, trade, historical}
  end

  def handle_call({:get_last_trades, products}, _source, historical) do
