defmodule Poeticoins.Exchanges.Client do
  ...
  def init({module, currency_pairs}) do
    ...
    # continue/connect tuple tells the GS to continue init process
    {:ok, client, {:continue, :connect}}
  end

  # this is called async after the init
  def handle_continue(:connect, client) do
    # connect to websockets & update state
  ...
  def handle_info({:gun_down, pid, :ws, reason, _unknown1, _unknown2}, %{conn: _conn}=client) do
    IO.inspect("Connection with Pid:#{pid} downed with reason: #{reason}", label: "Gun conn down.")
    {:noreply, client}
  end
  ...
  def handle_info({:gun_ws, conn, _ref, {:text, msg}=_frame}, %{conn: conn}=client) do
    handle_ws_msg(Jason.decode!(msg), client)
  end
  ...
  defp handle_ws_msg(msg, client) do
